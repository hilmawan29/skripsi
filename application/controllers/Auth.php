...
{ 
	...
	public function index () {
		$data['title'] = 'Role';
		$data['list_role'] = $this->role->selectAll();
		...
		if($user){
			if ($user['is_active'] == 1) {
				if (password_verify($password, $user['password'])) {
					...
					redirect('auth/login_form?role_id='.$role_id);	
				}	
			}else{
			...
			$this->session->set_flashdata('message', '<div class="alert alert-danger" role="alert">Email is not registered in this role!</div>');
			redirect('auth/login_form?role_id='.$role_id);
		}
	}
	...
	public function registration () {
		...
		$this->form_validation->set_rules('email', 'Email', 'required|trim|valid_email|is_unique[user.email]', [
			'is_unique' => 'This email has al registered!'
		]); 
